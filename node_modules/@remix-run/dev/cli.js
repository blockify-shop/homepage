...
switch (cli.input[0]) {
  case "routes":
    commands__namespace.routes(cli.input[1], cli.flags.json ? "json" : "jsx").catch(handleError);
    break;

  case "build":
    if (!process.env.NODE_ENV) process.env.NODE_ENV = "production";
    commands__namespace.build(cli.input[1], process.env.NODE_ENV, cli.flags.sourcemap).catch(handleError);
    break;

  case "watch":
    if (!process.env.NODE_ENV) process.env.NODE_ENV = "development";
    commands__namespace.watch(cli.input[1], process.env.NODE_ENV).catch(handleError);
    break;
  ...
  case "dev":
    if (!process.env.NODE_ENV) process.env.NODE_ENV = "development";
    commands__namespace.dev(cli.input[1], process.env.NODE_ENV).catch(handleError);
    break;

  default:
    // `remix ./my-project` is shorthand for `remix dev ./my-project`
    if (!process.env.NODE_ENV) process.env.NODE_ENV = "development";
    commands__namespace.dev(cli.input[0], process.env.NODE_ENV).catch(handleError);
}
